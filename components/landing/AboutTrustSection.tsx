...
function AboutCard({ title, description, icon }: { title: string; description: string; icon: "vision" | "values" }) {
  return (
    <article className="group/about-card relative min-h-[154px] overflow-hidden rounded-[12px] border border-[#4a4d55] bg-[linear-gradient(135deg,rgba(40,43,49,0.96)_0%,rgba(30,32,38,0.95)_52%,rgba(34,34,34,0.95)_100%)] px-5 py-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.03),0_12px_32px_rgba(0,0,0,0.18)] transition duration-300 hover:border-[#e2b13f]/45 hover:shadow-[inset_0_1px_0_rgba(255,255,255,0.06),0_0_18px_rgba(219,168,51,0.14),0_18px_44px_rgba(0,0,0,0.32)] md:min-h-[154px] md:px-5 md:py-5">
      ...
      <h3 className="relative mt-5 text-[#f4f4f4]" style={{ fontFamily: "var(--font-poppins), Poppins, sans-serif", fontSize: "15px", lineHeight: "1.25", fontWeight: 500 }}>
        {title}
      </h3>
      <p className="relative mt-3 max-w-[330px] text-[#b6b6b8]" style={{ fontFamily: "var(--font-poppins), Poppins, sans-serif", fontSize: "11px", lineHeight: "1.7" }}>
        {description}
      </p>
    </article>
  );
    ...
    >
      ...
      <div className="relative mx-auto max-w-[1320px]">
        <motion.div
          ...
          variants={{ hidden: { opacity: 0, x: -90, filter: "blur(8px)" }, show: { opacity: 1, x: 0, filter: "blur(0px)", transition: { duration: 0.72, ease: [0.16, 1, 0.3, 1] } } }}
        >
          <span className="h-px w-5 bg-[#b08a46]" />
          <p className="text-[9px] font-medium uppercase tracking-[0.36em] text-[#b08a46]" style={{ fontFamily }}>
        ...
        <motion.div
          ...
          variants={{ hidden: { opacity: 0, y: 54, scale: 0.95, filter: "blur(10px)" }, show: { opacity: 1, y: 0, scale: 1, filter: "blur(0px)", transition: { duration: 0.86, ease: [0.16, 1, 0.3, 1], delay: 0.08 } } }}
        >
          <h2 className="text-white" style={{ fontFamily, fontSize: "clamp(24px, 2.9vw, 34px)", lineHeight: "1.15", fontWeight: 600, letterSpacing: "-0.04em" }}>
            {t("about.heading")}
          </h2>
          <p className="mx-auto mt-5 max-w-[430px] text-left text-white/52 sm:text-center" style={{ fontFamily, fontSize: "11px", lineHeight: "1.5", letterSpacing: "0.02em" }}>
            {t("about.description")}
          </p>
        </motion.div>
        ...
        <div className="mx-auto mt-14 grid max-w-[804px] gap-4 md:mt-16 md:grid-cols-2 md:gap-[14px]">
          <div className="md:mr-[8px]">
            <motion.div variants={{ hidden: { opacity: 0, x: -150, filter: "blur(12px)" }, show: { opacity: 1, x: 0, filter: "blur(0px)", transition: { duration: 0.9, ease: [0.16, 1, 0.3, 1], delay: 0.2 } } }}>
              <AboutCard icon="vision" title={t("about.vision.title")} description={t("about.vision.description")} />
            </motion.div>
          </div>
          <div className="md:ml-[8px] md:translate-y-[42px]">
            <motion.div variants={{ hidden: { opacity: 0, x: 150, filter: "blur(12px)" }, show: { opacity: 1, x: 0, filter: "blur(0px)", transition: { duration: 0.9, ease: [0.16, 1, 0.3, 1], delay: 0.32 } } }}>
              <AboutCard icon="values" title={t("about.values.title")} description={t("about.values.description")} />
            </motion.div>
          </div>
        </div>
      </div>
